...
pub fn readSiteFile(allocator: std.mem.Allocator, logFileWriter: std.fs.File.Writer, siteName: []const u8, blk2a: *Blk2a, blkc: *Blkc, nhw: u32, nvn: u32, nhe: u32, nvs: u32) anyerror!void {
    ...
    // Open site file
    const fs = std.fs.cwd();
    const siteFile = fs.openFile(siteName, .{}) catch |err| {
    ...
    while (true) {
        ...
        const nh1 = try parseTokenToInt(u32, error.InvalidGridCellPositionInSiteFile_W, tokens.items[0], logFileWriter) - 1;
        const nv1 = try parseTokenToInt(u32, error.InvalidGridCellPositionInSiteFile_N, tokens.items[1], logFileWriter) - 1;
