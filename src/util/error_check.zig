...
///This struct returns model run completion time with success message if the run encounters no error(s), otherwise it returns model run failure message.
pub const CompletionTime = struct {
    ...
    pub fn success(self: *CompletionTime) !void {
        ...
        try self.err_log.print(
            "success: Ecosys model run in {s} completed in {d} {s}!\n",
            .{ self.runfile, elapsed.value, elapsed.unit },
        );
        try self.err_log.flush();

        std.debug.print(
            "\x1b[1;32msuccess: Ecosys model run in {s} completed in {d} {s}!\x1b[0m\n",
            .{ self.runfile, elapsed.value, elapsed.unit },
        );
    }
